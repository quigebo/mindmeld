module Llm
  class MemorySynthesisService
    ...
    def initialize(story)
      @story = story
      @memory_comments = story.comment_threads.memory_worthy.chronological
    end
    ...
    def perform_synthesis
      chat = RubyLLM.chat(model: 'gpt-5').with_temperature(1.0)
    ...
    def create_synthesized_memory(synthesis)
      content = synthesis.content.with_indifferent_access
      ...
      memory.update!(
        ...
        metadata: {
          ...
          generation_details: {
            model_used: synthesis.model_id,
            generated_at: Time.current,
            total_memories: @memory_comments.size,
            memory_types: @memory_comments.map { |c| c.llm_analysis&.dig('memory_type') }.compact.uniq,
            input_token_count: synthesis.input_tokens,
            output_token_count: synthesis.output_tokens,
            total_token_count: synthesis.input_tokens + synthesis.output_tokens
          }
        }
      )
